    ...
    public class DBModule
    {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(dbContext.Database.Connection.ConnectionString))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        temp.SerialNumber = reader.IsDBNull(16) ? null : reader.GetString(16);
                        temp.ConversionCounter = reader.IsDBNull(17) ? null : reader.GetString(17);
                        returnList.Add(temp);
                    }
                    return returnList;
                }
        ...
        public bool InsertStocktaking(Stocktaking temp)
        {
            try
            {
                ...
                {
                    conn.Open();
                    if (IsRemainInDB(temp.StocktakingID, conn))
                    {
                        DeleteStocktaking(temp.StocktakingID);
                        InsertStocktakingHelper(temp);
                        return true;
                    }
                    else
                    {
        ...
        private bool InsertStocktakingHelper(Stocktaking temp)
        {
            try
            {
                Entities dbContext = new Entities();
                ...
                //using (SqlConnection _con = new SqlConnection(APIConstant.Instance._connectionString))
                {
                    ...
                    SqlCommand _insert_command = new SqlCommand(APIConstant.Instance._insert_query, conn);
        ...
        public void DeleteStocktaking(string stocktakingID)
        {
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(string.Format(APIConstant.Instance._delete_query, stocktakingID), conn);
                    cmd.CommandType = CommandType.Text;
                    cmd.ExecuteNonQuery();
                }
            }
